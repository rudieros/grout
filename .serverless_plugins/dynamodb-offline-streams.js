...
class ServerlessOfflineDynamoDBStreams {
  ...
  getConfig() {
    return assignAll([
      ...
      omitBy(
        isUndefined,
        get(['custom', 'serverless-offline-dynamodb-streams'], this.service)
      ),
    ])
  }
  ...
  eventHandler(streamARN, functionName, shardId, Records, cb) {
    this.serverless.cli.log(
      `${extractTableNameFromARN(streamARN)} (λ: ${functionName})`
    )
  ...
  getTableName(tableEvent) {
    ...
    if (
      typeof tableEvent === 'string' &&
      startsWith('arn:aws:dynamodb', tableEvent)
    )
      return extractTableNameFromARN(tableEvent)
    if (typeof tableEvent.arn === 'string')
      return extractTableNameFromARN(tableEvent.arn)
